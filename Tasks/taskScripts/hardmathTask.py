...
import random
import re
import sys  # to get file system encoding (used in setDir())
import time
...
def load_instruction(PATH: str) -> list:
    ...
    with codecs.open(
        os.path.join(os.path.join(os.getcwd(), "resources/Maths_Task"), PATH),
        "r",
        encoding="utf8",
    ) as f:
        input_data = f.read()
...
class instructionsc(object):
    ...
    def __init__(self, window: visual.Window, instruction_txt: str):
        self.window = window
...
def block_generator(
    dfile: str, difficulty: int = 1, block_num: int = 4, trial_num: int = 6
) -> list:
    """
    Generate a list of blocks of trials.

    Args:
        dfile: The path to the csv file containing the stimuli.
        difficulty: The difficulty level of the stimuli.
        block_num: The number of blocks.
        trial_num: The number of trials in each block.

    Returns:
        A list of blocks of trials.
    """
    # path = 'taskScripts/resources/Maths_Task/new_math_stimuli' + str(difficulty) + '.csv'
    path = dfile
    stimFile = pd.read_csv(
        os.path.join(
            os.path.join(
                os.getcwd(),
            ),
...
def block_remover(blockSet):
    """
    This function removes the block structure from the data.

    Args:
        blockSet (list): A list of blocks.

    Returns:
        list: A list of trials.
    """
    result = []
    i = 0
...
def new_csv_creator(dictList):
    """
    This function takes a list of dictionaries and creates a new csv file.
    The new csv file is created by taking the keys from the dictionaries and
    creating a new column for each key. The values from the dictionaries are
    then added to the new csv file.
...
def runexp1(timer, win, writer, resultdict, data, runtime):
    ...
    # in seconds previous four fixation is 15s
    posttrialFixDur = 16.00
    # in seconds the final fixation is 16s
    ...
    def resultdictWriter(timepoint, timer, writer, iscorrect=None):
        resultdict["Timepoint"], resultdict["Time"], resultdict["Is_correct"] = (
            timepoint,
            timer.getTime(),
            iscorrect,
        )
    ...
    def load_conditions_dict(conditionfile):
        ...
        with open(os.path.join(os.getcwd(), conditionfile)) as csvfile:
            ...
            fieldnames = (
                reader.fieldnames
            )  # filenames is the first row, which used as keys of trials
        return (
            trials,
            fieldnames,
        )  # trial is a list, each element is a key-value pair. Key is the
    ...
    def trigger_exp(path, trigger_figure):
        ...
        trigger = visual.ImageStim(
            win, image=os.path.join(path, trigger_figure), pos=instru_pos
        )
        trigger.draw()
        win.flip()
    ...
    def ready(path, ready_figure):
        ...
        ready_dis = visual.ImageStim(
            win, image=os.path.join(path, ready_figure), pos=instru_pos
        )
        ready_dis.draw()
    ...
    def run_stimuli(stimuli_file, runtime):
        ...
        run_onset = (
            win.flip()
        )  # this is when the real experiment starts and the run starts
        ...
        for trial in all_trials:
            ...
            if curtime.getTime() < runtime:
                ...
                choice_right = prep_cont(
                    trial["choice"][len(trial["choice"]) - 4 : :], choice_left_pos
                )
                ...
                trial_onset = (
                    win.flip()
                )  # when expression is displayed, this is the trial onset
                ...
                keys = event.waitKeys(
                    maxWait=timelimit_deci, keyList=["left", "right"], timeStamped=True
                )
                fixa.draw()
                win.flip()
    ...
    try:
        ...
        with open(
            os.path.join(os.getcwd(), "taskScripts/resources/Maths_Task/instr3.txt")
        ) as f:
            lines3 = f.read()
            for i, cur in enumerate([lines1, lines2, lines3]):
                text_inst.setText(cur)
